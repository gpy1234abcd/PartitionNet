...
import datetime
import os
# os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # 指定使用显卡设备0
import numpy as np
import torch
...
if __name__ == "__main__":
    ...
    #目标的深度信息
    train_depth_annotation_path='2012_train_depth.txt'
    val_depth_annotation_path = '2012_val_depth.txt'
    #------------------------------------------------------#
    #   设置用到的显卡
    #------------------------------------------------------#
    ...
    if backbone == "resnet101":
        ...
        model = CenterNet_Resnet101(num_classes, pretrained = pretrained)
    else:
    ...
    with open(train_annotation_path) as f:
        ...
        # print(train_lines)
        #
        # train_lines=train_lines.replace(train_lines[0:34],'.')
        # print(train_lines)

    with open(val_annotation_path) as f:
        val_lines   = f.readlines()
    # with open(train_depth_annotation_path) as f:
    #     train_depth_lines = f.readlines()
    # with open(val_depth_annotation_path) as f:
    #     val_depth_lines  = f.readlines()
    #
    # #  # 把目标的深度值添加到annotation中
    # for l in range(len(train_lines)):
    #     # train_lines[l]=train_lines[l].replace(train_lines[l][0:35],'.')
    #
    #     train_lines[l] =train_lines[l].replace('\n', '')+' '+train_depth_lines[l].replace(" ","")
    # for ll in range(len(val_lines)):
    #
    #     # val_lines[ll]=val_lines[ll].replace(val_lines[ll][0:35],'.')
    #     val_lines[ll]  =val_lines[ll].replace('\n', '')+' '+val_depth_lines[ll].replace(" ","")
