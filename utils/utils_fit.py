...
import torch
from nets.centernet_training import focal_loss, reg_l1_loss,relation_loss,reg_l1_whdepth_loss,depth_ranking_loss
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            if backbone=="resnet50":
                ...
                hm,whdepth,offset = model_train(batch_images)


                # plt.imshow(hdepth[0, 0, :,:].cpu().detach().numpy())
                # plt.colorbar()
                # plt.show()

                # for i in range(20):
                #     plt.imshow(hm[0, i, :,:].cpu().detach().numpy())
                #     plt.colorbar()
                #     plt.show()


                # for i in range(3):
                #     plt.imshow(whdepth[0, i, :,:].cpu().detach().numpy())
                #     plt.colorbar()
                #     plt.show()


                # print(batch_hms.shape)
                # print(batch_depths.shape)
                # for i in range(20):
                #     plt.imshow(batch_hms[0, :, :,i].cpu().detach().numpy())
                #     plt.colorbar()
                #     plt.show()
                # for i in range(3):
                #     plt.imshow(batch_depths[0, :, :,i].cpu().detach().numpy())
                #     plt.colorbar()
                #     plt.show()

                c_loss          = focal_loss(hm, batch_hms)
                wh_loss         = reg_l1_whdepth_loss(whdepth, batch_whs, batch_reg_masks)


                ranking_loss    = depth_ranking_loss(whdepth, batch_whs, batch_reg_masks)
                off_loss        = reg_l1_loss(offset, batch_regs, batch_reg_masks)
                loss            = c_loss + ranking_loss+0.1*wh_loss+off_loss


                total_loss            += loss.item()
                total_c_loss          += c_loss.item()
                total_ranking_loss    +=ranking_loss.item()
                # total_ranking_loss    += torch.from_numpy(np.array(ranking_loss.cpu().detach().numpy())).item()
        ...
        if local_rank == 0:
            pbar.set_postfix(**{'total_c_loss'  : total_c_loss / (iteration + 1),
                                'total_ranking_loss'  : total_ranking_loss / (iteration + 1),
