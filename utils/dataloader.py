...
def draw_cricle(heatmap, center, radius, k=1):
    radius = 6
    diameter = 2 * radius + 1
    gaussian = gaussian2D((diameter, diameter), sigma=diameter / 6)
...
def gaussianone(shape, sigma=1):
    m, n = [(ss - 1.) / 2. for ss in shape]
    y, x = np.ogrid[-m:m + 1, -n:n + 1]

    h = np.exp(-(x * x + y * y) / (2 * sigma * sigma))
    h[h < np.finfo(h.dtype).eps * h.max()] = 0
    h=np.ones((int(m),int(n)),dtype=int)
    return h
...
class CenternetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #   进行数据增强
        #-------------------------------------------------#
        # depth是图像上目标的深度数值 depth是字符串类型
        # print("@@@@")
        image,box,depth = self.get_random_data(self.annotation_lines[index], self.input_shape, random = self.train)
        #depth 本来是字符串类型，变成list 方便下面的编程
        depth=eval(depth)

        batch_hm        = np.zeros((self.output_shape[0], self.output_shape[1], self.num_classes), dtype=np.float32)
        # batch_depth     = np.zeros((self.output_shape[0], self.output_shape[1], 3), dtype=np.float32)
        batch_wh        = np.zeros((self.output_shape[0], self.output_shape[1], 3), dtype=np.float32)
        ...
        for i in range(len(box)):
            ...
            if h > 0 and w > 0:
                ...
                #   计算真实框所属的特征点
                #-------------------------------------------------#
                ct = np.array([(bbox[0] + bbox[2]) / 2, bbox[3]], dtype=np.float32)
                ...
                # 计算宽高真实值
                #---------------------------------------------------#
                batch_wh[ct_int[1], ct_int[0]] = 1. * w, 1. * h,1.*depthhh
                #---------------------------------------------------#
                # #   计算中心偏移量
                # #---------------------------------------------------#
        ...
        # return image, batch_hm, batch_depth,  batch_reg_mask
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        # print(len(line))
        # print(line[0]) 其中存储了图像的路径
        # print(line[-1]) 存储了图像的目标的深度值
        # print(line[1:-1]) 目标的坐标和类别
        #------------------------------#
        #   读取图像并转换成RGB图像
        #------------------------------#
        ...
        return image_data, box,depth_obj
...
def centernet_dataset_collate(batch):
    ...
    # batch_pairs     = torch.from_numpy(np.array(batch_pairs).astype(float)).type(torch.FloatTensor)
