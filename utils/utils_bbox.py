...
def decode_bbox(pred_hms, pred_whs, pred_offsets, confidence, cuda):
    ...
    for batch in range(b):
        ...
        pred_wh     = pred_whs[batch].permute(1, 2, 0).view([-1, 3])
        ...
        compleat_h = pred_wh_mask[..., 1:2]
        # 预测目标的深度值
        depth = pred_wh_mask[..., 2:3]

        #----------------------------------------#
        #   获得预测框的左上角和右下角
        #----------------------------------------#
        # bboxes = torch.cat([xv_mask - half_w, yv_mask - half_h, xv_mask + half_w, yv_mask + half_h], dim=1)
        bboxes = torch.cat([xv_mask - half_w, yv_mask - compleat_h, xv_mask + half_w, yv_mask], dim=1)
        bboxes[:, [0, 2]] /= output_w
        bboxes[:, [1, 3]] /= output_h
        detect = torch.cat([bboxes, torch.unsqueeze(class_conf[mask],-1), torch.unsqueeze(class_pred[mask],-1).float(),depth], dim=-1)
